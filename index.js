...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\nPor favor seja ativo qualquer duvida de .help🥀🖤`
				...
				teks = `Tchau @${num.split('@')[0]} foi tarde😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ espera bb to fazendo... ⌛',
				success: '✔️ ta mec irmão deu certo ✔️',
				error: {
					stick: '⚠️ Falha, ocorreu um erro ao converter a imagem em figurinha ⚠️',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
					group: '❌ sai do meu pv seu tchola❌',
					ownerG: '⚠️ Este comando só pode ser usado pelo dono do bot! 😂',
					ownerB: '❌ Este comando só pode ser usado pelo proprietário do bot! ❌',
					admin: '⚠️ so pra adm parceiro! 😝',
					Badmin: '❌ Este comando só pode ser usado quando eu virar adm porra! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["557791235791@s.whatsapp.net"] //557791235791
			const isGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await client.groupMetadata(from) : 'grupo da baderninha'
			const groupName = isGroup ? groupMetadata.subject : '𝐁𝚯𝐓 𝐉𝚯𝐀̃𝚯 𝐃𝐀𝐗 𝐏𝐔𝐓𝚫🇪🇨'
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'lista dos bloquado q vem encher o saco no pv :\n'
				...
				case 'gtts':
					...
					if (args.length < 2) return client.sendMessage(from, 'coloca o texto burro', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
        ...
        case 'promote':
				...
				case 'listadmins':
					...
					break
                                case 'linkgroup':
                                        ...
                                        reply('https://chat.whatsapp.com/LAg2ne6ARAu48QlpAB1SDO'+linkgc)
                                        break
                                case 'leave':
                                        if (!isGroup) return reply(mess.only.group)
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ falhou amigão jaozin pede desculpas ❌')
					...
					break
				case 'simi':
					if (args.length < 1) return reply('me da o cu')
					teks = body.slice(5)
